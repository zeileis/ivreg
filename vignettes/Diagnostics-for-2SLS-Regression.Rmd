...
The **ivreg** package extends a variety of standard numeric and graphical regression  diagnostics to linear models fit by *two-stage least-squares (2SLS)* regression, a commonly employed method of instrumental-variables estimation for potentially overidentified structural equations in which there are endogenous regressors. The `fitivreg2()` function in the package computes the 2SLS estimator employing a low-level interface not generally intended for direct use, and returns a list containing quantities that faciliate the computation of various diagnostics. The `ivreg2()` function provides a user-friendly formula-based interface to `fitivreg2()`.

`ivreg2()` is broadly similar in its usage to the `tsls()` function in the **sem** package and the `ivreg()` function in the **AER** package. Its raison-d'etre therefore is the provision of regression diagnostics. The subject of this vignette is the rationale for the various diagnostics and the use of functions in the **ivreg** package to compute them, along with functions in other packages, specifically the base-R **stats** package and the **car** and **effects** packages [@FoxWeisberg2019], that work with the `"ivreg2"` objects produced by `ivreg2()`.
...
Belsley, Kuh, and Welsch specifically examine (in my notation) the values of $\mathrm{dfbeta}_i = b_{\mathrm{2SLS}} - b_{\mathrm{2SLS}-i}$. They discuss as well the deleted values of the residual standard deviation $s_{-i}$. (Belsley, Kuh, and Welsch define the residual variances $s^2$ and $s_{-i}^2$ respectively as the full-sample and deleted residual sums of squares divided by $n$; in the **ivreg** packages, I divide by the residual degrees of freedom, $n - p$ for the full-sample value of $s^2$ and $n - p - 1$ for the case-deleted values.)
...
The **ivreg** package implements unusual-data diagnostics for 2SLS regression (i.e., class `"ivreg2"` objects produced by `ivreg2()`) as methods for various generic functions in the **stats** and **car** packages; these functions include `cooks.distance()`, `dfbeta()`, `hatvalues()`, `influence()`, and `rstudent()` in **stats**, and `avPlot()` and `qqPlot()` in **car**. In particular, `influence.ivreg2()` returns an object containing several diagnostic statistics, and it is thus more efficient to use the `influence()` function than to compute the various diagnostics separately. Methods provided for class `"influence.ivreg2"` objects include `cooks.distance()`, `dfbeta()`, `hatvalues()`, `qqPlot()`, and `rstudent()`.
...
The structural equations are estimated as follows by the `ivreg2()` function [compare @Kmenta1986, p. 686]:
```{r}
deq <- ivreg2(Q ~ P + D, ~ D + F + A, data=Kmenta)     # demand equation
summary(deq)
```
```{r}
seq <- ivreg2(Q ~ P + F + A, ~ D + F + A, data=Kmenta) # supply equation
summary(seq)
```

`"ivreg2"` objects inherit from class `"lm"`, and many methods for `"lm"` objects work properly for `"ivreg2"` objects. For example, the `plot()` method for `"lm"` objects produces interpretable plots, here for the 2SLS fit for the demand equation in Kmenta's model:
...
* If, alternatively, we know the variance of the errors up to a constant of proportionality, then we can use inverse-variance weights for the 2SLS estimator. The `ivreg2()` function supports weighted 2SLS regression, and the diagnostics in the **ivreg** package work with weighted 2SLS fits  (see the next section).
...
The **ivreg** package supports the `sandwich()` function in the **sandwich** package. For the Kmenta example, where evidence of nonconstant error variance is slight, the sandwich standard errors are similar to, indeed slightly smaller than, the conventional 2SLS standard errors:
```{r}
...
$$
Alternatively, we can treat the two stages of 2SLS as *weighted least squares (WLS)* problems, in each stage minimizing the weighted sum of squared residuals. The `ivreg2()` function computes the weighted 2SLS estimator in this manner.
