...
Most of the diagnostic and other methods for `"ivreg"` objects also work with robust two-stage regression models. For example, here is a graph of hatvalues, studentized residuals, and Cook's distances:
```{r}
influencePlot(deq.mm)
```
Because the hatvalue for 1941 in the robust fit is 0, so is the Cook's $D$ for this case, and the "bubble" for 1941 has 0 area (and is therefore invisible). The bad case, however, have a very large (negative) studentized residual of nearly $-8$.
