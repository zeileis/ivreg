...
m.ivreg <- with(Kmenta, fitivreg(Q, cbind(1, P, D), cbind(1, D, F, A)))
...
se.ivreg <- as.vector(sqrt(diag(m.ivreg$vcov)))
...
m.ivreg.w <- with(Kmenta, fitivreg(Q, cbind(1, P, D), cbind(1, D, F, A), wt=Q))
...
se.ivreg.w <- as.vector(sqrt(diag(m.ivreg.w$vcov)))
